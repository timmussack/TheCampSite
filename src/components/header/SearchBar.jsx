...
function SearchBar() {
  ...
  return (
    <Autocomplete
      className="border-primary border rounded-2xl pl-3 sm:h-9 sm:w-56 desktop:mt-0 self-center"
      placeholder="Search by location..."
      apiKey={`${googleApi}`}
      options={{ types: ['locality', 'administrative_area_level_3', 'street_address', 'park'] }}
      onPlaceSelected={(place) => { coords(place); }}
    />
  );
}
